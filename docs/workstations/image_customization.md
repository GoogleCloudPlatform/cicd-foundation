...
_Bypass_: Pass `"NOCHECK"` natively as the `sha256` parameter to instantly bypass explicit checksum validation completely when necessary.
...
_Note: Designed to avoid Alpine Busybox `tar` parsing breakages._
...
## 4. Taxonomy: Workstations vs DevContainers

The repository strictly separates full Google Cloud Workstation custom container images from standalone Development Containers:

- **`apps/workstations/` (Cloud Workstations Custom Images)**:

  - Custom container images serving as the host VM/container for Cloud Workstations instances.
  - Contain systemd init, Nginx preflight proxy gateway, SSH daemon, user session managers, Docker daemon (Docker-in-Docker), and desktop or IDE runtime layers (e.g. `codeoss`, `gnome`, `codeoss-devcontainer-intellij`, `codeoss-devcontainer-gvisor-terraform`).
  - Defined with a `Dockerfile` and `skaffold.yaml` building full workstation images.

- **`apps/devcontainers/` (Standalone Developer Containers)**:
  - Portable development container definitions according to the Development Containers specification (`.devcontainer/devcontainer.json`).
  - Meant to be executed by developers **inside** a running Cloud Workstation instance (via Code OSS, IntelliJ, or CLI) or locally via `@devcontainers/cli`.
  - Built using `build_devcontainer $IMAGE` in `skaffold.yaml` with the platform's `build-runner`.
