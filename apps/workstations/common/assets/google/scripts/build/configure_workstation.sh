...
main() {
  ...
  for ssh_unit in /lib/systemd/system/ssh.service /usr/lib/systemd/system/ssh.service /etc/systemd/system/ssh.service; do
    if [[ -f "${ssh_unit}" ]]; then
      ln -sf "${ssh_unit}" /etc/systemd/system/multi-user.target.wants/ssh.service
      break
    fi
  done
